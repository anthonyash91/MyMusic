...
export default function ArtistPage({
  ...
  const [currentDiscographyLink, setCurrentDiscographyLink] =
    useState("Popular Releases");
        ...
        <>
          ...
          artistInfo.data.artist.visuals.headerImage.sources.length ? (
            <div
              id="artist-header"
              className={
                artistInfo.data.artist.visuals.headerImage.sources[0]
                  ? "expand-height"
                  : ""
              }
              style={{
                backgroundImage:
                  "url(" +
                  artistInfo.data.artist.visuals.headerImage.sources[0].url +
                  ")",
              }}
            >
              ...
              <div id="artist-info">
                ...
                artistInfo.data.artist.visuals.avatarImage.sources.length ? (
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "url(" +
                        artistInfo.data.artist.visuals.avatarImage.sources[0]
                          .url +
                        ")",
                    }}
                  ></div>
                ...
                <div id="name-and-stats">
                  <div className="artist-name">
                    {artistInfo.data.artist.profile.name}
                  </div>

                  <div id="stats">
                    <span>
                      {artistInfo.data.artist.stats.monthlyListeners.toLocaleString()}{" "}
                      listeners
                    </span>
                    <span>&bull;</span>
                    <span>
                      {artistInfo.data.artist.stats.followers.toLocaleString()}{" "}
                      followers
                    </span>
                  </div>
            ...
            <div
              ...
              style={{
                backgroundColor:
                  artistInfo.data.artist.visuals.avatarImage.extractedColors
                    .colorRaw.hex,
              }}
            >
              <div id="artist-info">
          ...
          <div id="discography-menu">
            ...
            artistInfo.data.artist.discography.popularReleases.items.length ? (
              <span
                className={
                  currentDiscographyLink === "Popular Releases" ? "active" : ""
                }
                onClick={() => {
            ...
            artistInfo.data.artist.discography.albums.items.length ? (
              <span
                className={currentDiscographyLink === "Albums" ? "active" : ""}
                onClick={() => {
            ...
            artistInfo.data.artist.discography.singles.items.length ? (
              <span
                className={
                  currentDiscographyLink === "Singles & Eps" ? "active" : ""
                }
                onClick={() => {
            ...
            artistInfo.data.artist.discography.compilations.items.length ? (
              <span
                className={
                  currentDiscographyLink === "Compilations" ? "active" : ""
                }
                onClick={() => {
            ...
            artistInfo.data.artist.discography.topTracks.items.length ? (
              <span
                className={
                  currentDiscographyLink === "Top Tracks" ? "active" : ""
                }
                onClick={() => {
          ...
          <div id="discography">
            ...
            artistInfo.data.artist.discography.popularReleases.items.length ? (
              <div
                className={`discography-section ${
                  showDiscography === "Popular Releases" ? "show" : "hide"
                }`}
              >
                {artistInfo.data.artist.discography.popularReleases.items.map(
                  (release, i) => {
                    const { name, uri, coverArt } = release.releases.items[0];
                    return (
                      <div className="discography-album" key={i}>
                        {coverArt && coverArt.sources.length ? (
                          <Link
                            to={`/album/${uri.slice(-22)}`}
                            onClick={() => {
                              clearData([]);
                            }}
            ...
            artistInfo.data.artist.discography.albums.items.length ? (
              <div
                className={`discography-section ${
                  showDiscography === "Albums" ? "show" : "hide"
                }`}
              >
                {artistInfo.data.artist.discography.albums.items.map(
                  (album, i) => {
            ...
            artistInfo.data.artist.discography.singles.items.length ? (
              <div
                className={`discography-section ${
                  showDiscography === "Singles & Eps" ? "show" : "hide"
                }`}
              >
                {artistInfo.data.artist.discography.singles.items.map(
                  (single, i) => {
            ...
            artistInfo.data.artist.discography.compilations.items.length ? (
              <div
                className={`discography-section ${
                  showDiscography === "Compilations" ? "show" : "hide"
                }`}
              >
                {artistInfo.data.artist.discography.compilations.items.map(
                  (compilation, i) => {
                    const { name, uri, coverArt } =
                      compilation.releases.items[0];
                    return (
                      <div className="discography-album" key={i}>
                        {coverArt && coverArt.sources.length ? (
                          <Link
          ...
          artistInfo.data.artist.discography.topTracks.items.length ? (
            <div
              className={`discography-section ${
                showDiscography === "Top Tracks" ? "show" : "hide"
              }`}
            >
